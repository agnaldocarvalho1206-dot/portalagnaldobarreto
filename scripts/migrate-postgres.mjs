...
const pool = new Pool({ connectionString: process.env.MIGRATION_DATABASE_URL, max: 1, connectionTimeoutMillis: 5000, ssl: process.env.DATABASE_SSL === 'true' ? {rejectUnauthorized:true} : false });
let client;
try {
  ...
  console.log(`Migrações verificadas: ${files.length}`);
} catch (error) {
  if (client) await client.query('ROLLBACK');
  console.error('Migração não concluída. Confira conexão, permissões e checksums.');
  process.exitCode=1;
} finally {
  client?.release();
  await pool.end();
