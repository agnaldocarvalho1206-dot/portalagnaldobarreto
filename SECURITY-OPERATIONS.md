...
- Contadores atômicos no D1, em chaves rate: da tabela settings. Não fazem parte das configurações públicas. Contato: 5 tentativas/hora por IP; newsletter: 5/minuto por IP; portal e configurações: 30/minuto; mensagens: 10/minuto por usuário. Tentativas inválidas também contam.
...
1. Testar duas identidades de cliente e um administrador na hospedagem; verificar isolamento e substituição dos cabeçalhos de autenticação. Não transferir o cookie de login simulado local para produção.
...
5. Confirmar backup do D1 e R2. Definir RPO (perda máxima tolerada) e RTO (tempo máximo de recuperação); restaurar ambos em homologação e conferir relações entre arquivos e solicitações. Registrar data, responsável e resultado.
...
7. Auditar dependências e histórico de segredos antes de publicação; verificar configuração de ADMIN_EMAIL em ambiente de produção. O valor local não é transferido automaticamente.
