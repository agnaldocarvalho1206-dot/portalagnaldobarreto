...
export async function publicSettings(){const row=await rawDb().prepare('SELECT value FROM settings WHERE key = ?').bind('public').first<{value:string}>();return row?JSON.parse(row.value):{}}
