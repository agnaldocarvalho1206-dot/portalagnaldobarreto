...
async function databasePosts():Promise<PublicBlogPost[]>{
 try{
  const rows=(await rawDatabase().prepare("SELECT title,slug,summary,category,body,minutes,seo_title,seo_description,featured_image,published_at FROM portal_contents WHERE status='Publicado' AND content_type='Post' AND slug<>'' ORDER BY published_at DESC, updated DESC LIMIT 500").all()).results as any[];
