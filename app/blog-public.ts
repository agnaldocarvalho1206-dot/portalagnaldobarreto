...
export type PublicBlogPost={slug:string,title:string,category:string,summary:string,minutes:string,publishedAt:string,author:string,featured:boolean,tags:string[],imageIndex:number,image:string,body:string,seoTitle?:string,seoDescription?:string,seoKeywords?:string};
...
async function databasePosts():Promise<PublicBlogPost[]>{
 try{
  const rows=(await rawDatabase().prepare("SELECT title,slug,summary,category,body,minutes,seo_title,seo_description,featured_image,seo_keywords,published_at FROM portal_contents WHERE status='Publicado' AND content_type='Post' AND slug<>'' ORDER BY published_at DESC, updated DESC LIMIT 500").all()).results as any[];
  return rows.map((p,index)=>({slug:String(p.slug),title:String(p.title),category:String(p.category||'Estratégia'),summary:String(p.summary||''),minutes:String(p.minutes||'3 min'),publishedAt:String(p.published_at||''),author:'Agnaldo Barreto',featured:index<3,tags:[String(p.category||'Estratégia')],imageIndex:index%3,image:String(p.featured_image||'/blog-reference.png'),body:String(p.body||''),seoTitle:String(p.seo_title||''),seoDescription:String(p.seo_description||''),seoKeywords:String(p.seo_keywords||'')}));
