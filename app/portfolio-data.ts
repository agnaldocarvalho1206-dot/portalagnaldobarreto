...
export const portfolioItems=projects.map((p,index)=>({...p,order:index,thumbnail:'/portfolio-reference.png',imageAlt:'Projeto '+p.name,displayCategory:p.slug==='instituto-ive'?'ONG / Projeto social':p.category,filterCategory:p.category==='Institucional'?'Institucionais':p.category,concept:false}));
